...
def keycombo_to_pynput_str(key_combo):
    ...
    if sys.platform == "darwin":
        ...
        modifier_map[Qt.ControlModifier], modifier_map[Qt.MetaModifier] = (
            modifier_map[Qt.MetaModifier],
            modifier_map[Qt.ControlModifier],
        )
...
class HotkeyManager(QObject):
    ...
    def set_hotkey(self, name, key_combo):
        ...
        thread = keyboard.GlobalHotKeys(
            {
                keycombo_to_pynput_str(key_combo): lambda: self.hotkey_triggered.emit(
                    name, key_combo
                )
            }
        )
